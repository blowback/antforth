...
| `DELAY` | `timer.asm` | Non-standard (antforth extension — Story 24.2; `( u -- )` busy-wait `u` seconds. Computes `target = TICKS + u*64` as a double held on the caller's own data stack — no shared countdown cell — then spins until `TICKS` reaches it. Single-threaded form; the wait loop is factored into the internal `(DELAY)` helper so the Phase-6 multitasker can later drop one `PAUSE` in without changing the target math. `0 DELAY` returns at once. NOT a DPANS94 word) |
| `MS` | `timer.asm` | Forth-2012 §10.6.2.1905 (FACILITY EXT — Story 24.2; `( u -- )` "Wait at least `u` milliseconds." `target = TICKS + ceil(u*64/1000)` over the same 64 Hz counter; the tick count is rounded UP so any nonzero `u` waits ≥1 tick — the standard's implementation-defined resolution here is the 15.625 ms tick, so `MS` is a coarse convenience, not a true-millisecond timer. Shares the `(DELAY)` wait loop) |
