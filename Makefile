...
test-repl-banking: $(TARGET)
	...
	FAILED=0; \
	for pat in 'PASS: banking-emu-probe' 'PASS: banking-mapping-on-idempotent' 'PASS: banking-mapping-on-port-74' 'PASS: bank-at-zero' 'PASS: banks-zero' 'PASS: bank-store-abort-bank-q' 'SKIP: bank-store-round-trip-1' 'SKIP: bank-store-round-trip-0' 'INFO: bank-store-t-states'; do \
...
test-repl-banking-skip: $(TARGET)
	...
	FAILED=0; \
	for pat in '^SKIP: banking-emu-probe' '^SKIP: banking-mapping-on-port-74' 'PASS: banking-mapping-on-idempotent' 'PASS: bank-at-zero' 'PASS: banks-zero' 'PASS: bank-store-abort-bank-q' 'SKIP: bank-store-round-trip-1' 'SKIP: bank-store-round-trip-0'; do \
		if echo "$$OUTPUT" | grep -q "$$pat"; then \
