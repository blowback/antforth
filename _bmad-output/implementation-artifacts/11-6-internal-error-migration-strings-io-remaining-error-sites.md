...
| 13 | Section 4 added to `tests/throw_migration_tests.fth` covering -17 caught + i*x + DEPTH + positive controls + deferral notes for -58/-270/-271 caught | `tests/throw_migration_tests.fth:202-260`. Caveat: AC #13 only authorised falling back to uncaught-only for -58 if `EVALUATE` is absent; EVALUATE is present (`outer_interpreter.asm:387`) but `( unterminated " EVALUATE` reads past EVALUATE's source frame into the outer REPL line, which prevents the prescribed caught harness from returning -58. Tracked as F6 (Review-2) follow-up below. -270/-271 caught deferrals stand per Story 11.5 D2 rationale | **PARTIAL** (-58 caught form deferred outside spec authorisation; uncaught form covered by Test 751) |
