...
- `_bmad-output/planning-artifacts/architecture.md` (modified) — five new PD-P4-N sub-blocks (PD-P4-11..15); F4 Action paragraph extension for §9.6 documented-gotcha; F5 closure paragraph; tracking block (:607..614) marked CLOSED for §9.1/§9.3/§9.4/§9.5/§9.6; ~12 in-place cross-reference updates across Important Decisions, Conflict Points Identified, NFR-P4-12..17 paragraph, Decision Impact Analysis sequence + budget table, Files Explicitly NOT Touched, Requirements-to-Structure Mapping (Epic 22), Architecture Validation Results row, Gap Analysis Important gaps bullet, Phase-5+ candidates §9.1 line, How-to-extend bullet. CR-fix pass (2026-05-15): H1 `+BANK" cap?"` non-syntax replaced with `` `+BANK` past cap raises `ABORT" cap?"` `` at :995/:1010/:1109; H2 "12 banks at 0x35..0x3F" → "11 banks" at :304 (PD-P4-8) and :414 (PD-P4-14); M1 PD-P4-13 :400 "Story 17.5 (CL parser past-cap edge case)" → "Story 17.4 (CL-tail parser)"; M2 F5 heading :987 "Six" → "Five"; M3 PD-P4-15 :445 stale self-ref `:379` → `:483` (post-PD-insertion line for Epic-22 budget row).
...
- 2026-05-15 — CR-fix pass (Story 16.4 adversarial review, 7 findings: 2H · 3M · 2L). HIGH + MEDIUM fixed in `architecture.md` only — no `prd.md`, `epics-phase4-...md`, or `redesign-doc` edits required for the H/M set. **H1** invalid Forth syntax `` `+BANK" cap?"` `` propagated at :995/:1010/:1109 → replaced with `` `+BANK` past cap raises `ABORT" cap?"` `` (the binding decision form PD-P4-13 uses internally). **H2** off-by-one "12 banks at 0x35..0x3F" → "11 banks" at PD-P4-14:414 (binding closure inherited by Story 17.4) and PD-P4-8:304 (same doc consistency; redesign-doc :81 authoritative count "(11)"). **M1** PD-P4-13:400 misattributed "Story 17.5 (CL parser past-cap edge case)" — Story 17.5 is `.BANKS` minimal form per epic doc :561; the CL surface inherits from Story 17.4 (CL-tail parser). **M2** F5 heading :987 said "Six" while body says "Five remaining" + closure says "5 of 5 resolved" — heading aligned to body's "Five". **M3** PD-P4-15:445 self-ref `:379` (pre-insertion line for Epic-22 byte-budget row) → `:483` (actual post-PD-insertion line). LOW findings (L1 PRD `prd.md:529` FR-P4-13 still "3–5-byte" after PD-P4-11 pins 4 B; L2 `prd.md:58` "default 12 banks" same off-by-one as H2 but pre-existing in PRD) deferred — no behavioural impact, not in adversarial-review fix scope. Post-fix verification: `wc -c build/antforth.com` = 24,995 B (zero binary delta); `make check-doc-sync` exit 0, 31 advisories, 0 drift (unchanged); all five defect-strings grep-clean. Story status `review` → `done`.
