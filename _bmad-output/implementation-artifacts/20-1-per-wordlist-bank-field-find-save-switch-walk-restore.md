...
- **Bank-removal bucket scrub** (CR follow-up `code-review`, 2026-06-12): removing the old `current_bank>0` bucket-skip linked bank-N entries into the shared FORTH-WORDLIST buckets, but `-BANK`/`BANKS-CLEAR` did not scrub them — leaving stale window fat-pointers that a later FIND/WORDS would page in as garbage (wild-pointer chain walk). Fixed by `scrub_forth_buckets` (`src/dictionary.asm`), called from `-BANK`/`BANKS-CLEAR` (`src/banking.asm`) BEFORE the `active_pages[]`/`bank_count` mutation: it sweeps all 64 buckets, unlinking entries in the dropped bank and renumbering surviving pointers whose logical index shifts. Removal/renumber gates on the entry **address's** window-residence (not the bank byte — bank-0 defs carry byte 0 but live at fixed addresses); `BANKS-CLEAR` keeps bank-0 (home/portal) window entries since `active_pages[0]` persists. Regression guard: `probe-20.1-f` (emulator fixture + `disk/a/P201FIND.FTH`). Binary +263 B → 27888 B; gates green (975/0 · 61/0 · isolated 6/15/2/2 · 20-1 7 · 20-2 5 · 20-3 5 · straddle 3/3).
- **AC8 hardware smoke** — **PASS on real MicroBeast 2026-06-12** (transcript `beastty-20260612-010418.bin`): under `AntForth v3.0.5`, `INCLUDE P201FIND.FTH` → AC7 (a)–(e) **plus the new scrub guard (f)** all `result=-1`, suite-end present, no strands. Bank-aware FIND + bank-removal scrub verified on silicon. (Original 2026-06-10 run covered (a)–(e); the 2026-06-12 re-run adds (f).)
