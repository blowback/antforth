...
Status: done
<!-- 2026-05-17: review → done at Story 17.6 epic close-out gate. CR pass had closed cleanly on 2026-05-16 (all HIGH+MEDIUM blockers FIXED or explicit-DEFERRED; H4 FIXED on hardware; H3+M1 carried forward) but the status-flip was missed. M1 wording-soften applied to §"Forward inheritance pointers" per the CR's "soften at Story 17.6" instruction. Sprint-status row also flipped. Cross-ref: 17-6 verdict-table walk + close-out hygiene note. -->
...
- **Epic 19** (per-bank dictionary state — wording softened at Story 17.6 close-out per the M1 disposition):
  - The `(here, latest, wordlist_head)` triple swap mechanism *is wired* in Story 17.2, but the `wordlist_head` slot saves/loads `(forth_wordlist)` (= the `WORDLIST_NEXT` chain pointer per `src/wordlists.asm:336-337`), **NOT** the per-bank dictionary state's bucket array. The load-bearing per-bank dictionary state for Epic 19's compile-time semantics is the 64-entry hash bucket array at `forth_wordlist+2..+129` — which Story 17.2's swap does NOT touch.
  - Epic 19 **MAY** keep this triple-swap shape (treating wordlist_head as one of several per-bank cells, with a separate per-bank-LATEST-chain-walk discipline replacing bucket swap), **REPLACE** it (expand the per-bank entry from 6 B → 6+128 = 134 B; bank-table[] grows to 29 × 134 = 3,886 B; doesn't fit in `$D400`-region without re-shape), or **INVERT** it (rebase `current_wordlist` per bank, keeping one shared bucket array). The decision is Epic 19's at dev-pass start; Story 17.2's mechanism is **not load-bearing** for that choice.
  - The `here` / `latest` slots of the triple ARE load-bearing for Epic 19 (per-bank HERE + LATEST plumbing reads / writes these); only the `wordlist_head` slot is at-risk.
- **Epic 21** inherits:
...
| 2026-05-17 | **Close-out hygiene applied at Story 17.6 epic close-out gate.** (a) M1 wording-soften — §"Forward inheritance pointers" "Epic 18 inherits" block split into separate Epic 18 + Epic 19 entries; Epic 19 entry now explicitly states wordlist_head saves/loads `(forth_wordlist)` (WORDLIST_NEXT chain pointer) NOT the per-bank dictionary state's bucket array, and that Epic 19 MAY keep / REPLACE / INVERT the triple-swap shape (Story 17.2's mechanism is not load-bearing for Epic 19's design choice). H3 envelope disposition retroactively bound by Story 17.5 close Q6-a-extended accept-with-rationale per `project_epic17_envelope.md`. (b) Status flipped review → done (CR pass close-out clerical miss; all blockers had been dispositioned cleanly on 2026-05-16). Cross-reference: Story 17.6 Dev Notes §"Verdict-table walk" + §"Close-out hygiene". |
