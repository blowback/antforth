...
Status: review (dev-pass closed 2026-05-20; **+35 B kernel** [CR-corrected 2026-06-03 — dev-pass recorded +33 B/26759 B but the committed `c97f87d` source builds to 26761 B; commit subject says +35 B; see Dev Agent Record §"CR-pass corrections 2026-06-03"]; hardware UAT load-bearing for iron-spike per beastty-20260520-153439.bin + bank-0 probes A/B/C/H per beastty-20260522-103928.bin; AC1/AC2/AC3/AC4/AC5/AC6-A/B/C/D/E/H + AC7/AC8/AC9 satisfied; AC6-F/G deferred to forward-work "NEXT-via-EXECUTE chokepoint" rework; bank-N HW UAT verdict deferred to follow-on defect Story 19.3.1 per 2026-05-22 disposition — see Dev Agent Record §"Dev-Pass Outcome 2026-05-20" + Sub-7.5)

> **⮕ Architectural-debt re-homing (2026-06-03):** every "Story 19.5" anchor in this file (and the inherited-debt table) now resolves to **Epic 19.5 — Cross-bank dispatch stabilization (DTC threading rework)**, defined in `_bmad-output/planning-artifacts/epics-phase4-epics-16-22.md`. The earlier observation that "Story 19.5 doesn't actually exist" was correct at the time — it was a never-spawned placeholder; the DTC-threading + trampoline-fragility + HW-slot-2 + CATCH-cross-bank + bank-N-HERE-init debt is now an explicit ADR-first stabilization interlude epic. Epic 19 closes via Story 19.4 (antforth 3.0.3) shipping the verified *mechanism* with this debt documented as out-of-epic. Historical references below are preserved as-written.
...
- [x] **Task 6 — Three-test-surface sweep + binary delta + doc-sync** (AC: #8, #9) — close-out values 2026-05-20:
  ...
  - [x] Sub-6.5 `wc -c build/antforth.com`: **26761 B** (= 26726 baseline + **35 B** realised [CR-corrected 2026-06-03; dev-pass recorded 26759/+33]; AC8 ~70 B envelope: **50% used**, ~35 B headroom). Component delta (dev-pass intermediate figures): CREATE bank-N branch ≈ +30 B; DOES> bank-N CFA walk ≈ +3 B — these sum to +33 but the committed build is +35 B; the 2 B gap is unattributed (itemisation recorded against an intermediate build, not re-bisected at CR). Total verified by CR rebuild of `c97f87d`.
...
- [x] **Task 7 — Hardware smoke** (AC: #7)
  ...
  - [x] Sub-7.2 Hardware UAT 2026-05-20 (transcript `~/Downloads/beastty-20260520-153439.bin`) ran `INCLUDE P193IRON.FTH` on real MicroBeast at the committed kernel (+35 B / 26761 B [CR-corrected from +33]); all three sentinels emitted (`---iron-spike-19.3-start---`, `iron-spike-sentinel-12345-returned`, `---iron-spike-19.3-end---`). This is the load-bearing verdict for the iron-spike test on the shipped layout. Banner free-bytes `27257` matched the emulator output at the same kernel size, confirming the same binary was loaded.
...
Independent adversarial code-review pass (verified diff of `c97f87d` + worktree rebuild from source). Verdict: kernel mechanism is **correct** — `w_CREATE_cf`'s EXX-at-entry register discipline is sound (BC/DE are alt-set scratch, so omitting the PUSH BC/DE wrap is justified — *not* a repeat of 19.2's BC-clobber), and `w_PAREN_DOES_cf`'s bank-N CFA walk correctly wraps the `LD B,A` scratch in PUSH/POP BC and reads the flag into A before restoring TOS. Corrections applied:
- **H1 (binary delta wrong):** dev-pass recorded +33 B / 26759 B throughout; CR rebuild of `c97f87d` = **26761 B / +35 B** (matches the commit subject "+35 B"). All metric occurrences corrected above; the per-component intermediates (CREATE ≈+30, DOES> ≈+3) under-sum the realised +35 by 2 B (gap unattributed). Still within the ~70 B AC8 envelope (now 50% used).
- **M1 (File List gap):** `docs/dev_journal.md` was committed in `c97f87d` but omitted from the File List — now added, flagged as an unrelated bundled change.
- **H2 (forward-work scope):** Probe-19.3-F's DOVAR-target hang is a **distinct** root cause from the DTC-threading-through-stub-xt defect — the cross-bank sentinel-trampoline assumes a DOCOL/EXIT pair, which `DOVAR`/`DOCON`/code-word targets never provide. Both are now explicitly owned by **Epic 19.5** (the stabilization interlude that superseded the never-spawned "Story 19.5"; see the re-homing note at the top of this file and `epics-phase4-epics-16-22.md`). Epic 19.5's ADR scope updated to cover non-DOCOL cross-bank dispatch, not just DTC threading.

Status stays at `review` (Epic-19.5 cluster). M2 (bank-N CREATE hardware-unverified at 19.3 close → moved to Story 19.3.1) and M3 (iron-spike emulator gate weakened to SKIP-on-missing-end-sentinel, can mask post-success corruption) recorded as notes; not actioned.

### Dev-Pass Outcome 2026-05-20

Story 19.3 dev-pass closed 2026-05-20 with **status = review**, kernel +35 B (26726 → 26761) [CR-corrected 2026-06-03 from +33/26759], hardware UAT load-bearing for iron-spike. Initial attempt (2026-05-20 morning) ran into iz-cpm-banking layout-sensitivity at the 26748→26749 byte boundary causing iron-spike to hang inside the full banking_tests.fth probe sequence; that was first dispositioned as "defer Story 19.3 entirely" (mirror of Story 19.2 H5 precedent). Project lead correctly identified that disposition as incoherent — "Story 19.5" referenced in the H5 comment doesn't actually exist, and there's no realistic forward work that magically fixes the layout issue between now and a post-19.4 close-out point.

Subsequent hardware UAT (transcript `~/Downloads/beastty-20260520-153439.bin`, INCLUDE'd `P193IRON.FTH` on real MicroBeast with the committed +35 B kernel [CR-corrected from +33]) confirmed iron-spike PASSes cleanly on hardware — both `---iron-spike-19.3-start---` and `---iron-spike-19.3-end---` sentinels emitted. This re-classifies the emulator hang as a **iz-cpm-banking-only layout-sensitivity quirk** (same family as `project_phase4_banking_off_emulator`: hardware works, emulator doesn't model the trampoline EXIT-chain transitions correctly under certain layouts). Story 19.3 resumed under disposition: ship with hardware-authoritative iron-spike verdict; emulator iron-spike recipe gets SKIP-with-rationale path.
...
- **`src/compiler.asm:w_PAREN_DOES_cf` bank-N CFA walk** (≈+3 B; dev-pass intermediate build 26756 → 26759 — **NB: committed `c97f87d` source builds to 26761 B / +35 B total, verified at CR; the per-component intermediates under-sum by 2 B, gap unattributed**): replaces LATEST-based walk with `(latest_count_flags_addr)`-based walk + F_HAS_STUB_XT_CELL flag-discriminator (Story 19.2 Q2-γ scratch contract). Bank-0 / legacy entries: CFA = count_flags_addr + 1 + name_len (bit-identical to pre-edit). Bank-N>0 entries: +2 to skip the Story-19.2-Q1-α stub-xt cell. Closes the pre-Story-19.3 latent defect where (DOES>) on a bank-N>0 entry walked stub_addr+2 = garbage stub-body bytes.
...
| `wc -c build/antforth.com` | 26726 B | 26761 B | +35 B / ~70 B envelope | ✓ AC8 (50% used) [CR-corrected 2026-06-03 from 26759/+33] |
...
- AC8 binary delta: **+35 B / ~70 B envelope (50% used)** [CR-corrected 2026-06-03 from +33/47%; verified by CR rebuild of `c97f87d` = 26761 B].
...
- `docs/dev_journal.md` — **[added at CR 2026-06-03 — was committed in `c97f87d` but omitted from this File List]** a dev-journal note on Z80 `OUT`/`IN` operand-order convention. **Unrelated to Story 19.3's CREATE/DOES> scope** — an incidental change bundled into the story commit (M1 finding). No kernel impact.
...
- 2026-05-20 (dev-story, FINAL): re-applied kernel edits and ran hardware UAT. Transcript `~/Downloads/beastty-20260520-153439.bin` confirmed iron-spike PASSes at +33 B on real MicroBeast (all three sentinels emitted via `INCLUDE P193IRON.FTH`). Story 19.3 RESUMED with hardware-authoritative iron-spike verdict per AskUserQuestion 2026-05-20 option 1. Iron-spike runtime invocation MOVED out of inline `tests/banking_tests.fth` into isolated subprocess via `disk/a/P193IRON.FTH` (referenced by both `test-repl-banking` and `test-repl-banking-skip` iron-spike recipes); emulator recipes get 3-outcome logic (PASS / SKIP-with-rationale / FAIL). Bank-0 probes A/B/C/H appended to banking_tests.fth + Makefile probe-id loop extended. NEW `tests/banking_tests_19_3.fth` with bank-N probes D/E (PASS) + F/G (defer-sentinels per discovered architectural-defect: cross-bank EXECUTE on DOVAR-target hangs because sentinel-trampoline requires DOCOL/EXIT pairs). NEW Makefile target `test-repl-banking-isolated-19-3` (Sub-5.8 parallel-target). NEW hardware-smoke disk files `P193BK0.FTH` + `P193BKN.FTH` + `P193IRON.FTH`. AC6-F/G deferred to forward-work "NEXT-via-EXECUTE chokepoint" rework (no separate story spawned — captured in this Dev Agent Record + sprint-status annotation; the rework fits naturally into a future Phase-4 epic). Close-out values: test-repl 975/0/2; test-repl-banking 61/0/3; test-repl-banking-isolated 6/0; test-repl-banking-isolated-19-3 3 PASS + 2 DEFER; test-repl-banking-skip 25/0/3; check-doc-sync 31 advisory / 0 drift; kernel **26761 B (+35 / ~70 envelope = 50%)** [CR-corrected 2026-06-03 from 26759/+33/47%]. Status: ready-for-dev → in-progress → review.
- 2026-06-03 (CR pass): independent adversarial code review (verified diff + worktree rebuild of `c97f87d`). Kernel mechanism verdict: **correct** (EXX-at-entry discipline in `w_CREATE_cf` sound; `(DOES>)` bank-N walk correct; pre-existing bank-N `(DOES>)` defect properly surfaced+fixed). Corrections applied (see Dev Agent Record §"CR-pass corrections 2026-06-03"): **H1** — realised binary delta is **+35 B / 26761 B**, not the +33 B/26759 B recorded at dev-pass (CR rebuild matches commit subject; the prior `+33 B` mentions in the dated 2026-05-20 entries above are superseded by this entry); **M1** — `docs/dev_journal.md` (unrelated Z80 `OUT`/`IN` note) added to File List; **H2** — Probe-19.3-F's DOVAR-target hang recognised as a *distinct* cross-bank-trampoline-assumes-DOCOL/EXIT root cause and added to Epic 19.5's ADR scope. M2 (bank-N CREATE HW-unverified at 19.3 close → 19.3.1) + M3 (iron-spike emulator gate weakened to SKIP-on-missing-end-sentinel) recorded as notes. Status unchanged: `review` (Epic-19.5 cluster).
