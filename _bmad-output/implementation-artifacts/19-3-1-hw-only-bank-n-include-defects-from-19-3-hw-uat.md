...
- [x] **Task 5 — Hardware UAT** (AC: #5, #9) — load-bearing for Story 19.3 bank-N AC discharge
  ...
  - [x] Sub-5.2 Hardware UAT completed 2026-05-22 15:21:52 on real MicroBeast (`~/Downloads/beastty-20260522-152152.bin`). v3 binary (26850 B) — three INCLUDE'd files all ran cleanly.
...
- [x] **Task 6 — Four-test-surface sweep + binary delta + doc-sync** (AC: #8, #9)
  ...
  - [x] Sub-6.6 `wc -c build/antforth.com` = **26850 B** (+89 B vs baseline 26761 at v3; itemised in Dev Notes §"Pre-build byte itemisation"; within ≤ 100 B AC8 envelope; v1=+30 B reverted, v2=+22 B partial fix, v3=+89 B properly implements CP/M 2.2 0x1A-as-EOF semantics)
...
**Realised: +89 B kernel delta (26761 → 26850); within ≤ 100 B working envelope.**

| Component | Site | Bytes |
|---|---|---|
...
| **D1 v3 — `fcb_eof_seen` per-FCB array** | `src/file_access.asm:151` | 8 |
...
| **D1 v3 — `pool_acquire` + `pool_release` resets** | `src/file_access.asm:310..318, 401..409` | ~17 |
| **D2 fix — `build_header` LATEST reorder + bucket-skip** | `src/compiler.asm:359..401` | 6 |
| **Realised** | `wc -c build/antforth.com` Δ | **+89** |

Epic-19 cumulative at Story 19.3.1 close:
- Story 19.1: +20 B
- Story 19.2: +123 B
- Story 19.3: +33 B
- **Story 19.3.1: +89 B** (D1 v2 + D1 v3 + D2; revised up after the v3 0x1A-as-EOF semantics; within the ≤ 100 B story envelope)
- **Cumulative: 265 B / 300 B = 88%; ~35 B headroom remains for Story 19.4 close-out (0 B kernel — close-out gate only)**

The v3 fix is structurally correct per CP/M 2.2 §"Function 20" — 0x1A is the file's end-of-content marker, NOT just a line terminator. Antforth pre-19.3.1 was wrong here. The +89 B reflects a real spec-vs-implementation correctness gap (Lesson 13-B "surface, file, fix once known"). Per `feedback_kernel_ldir_estimate_overshoot` ×1.25 ± 10% empirical pattern, this story's realised 89 B is at the upper end (multi-component fix across file_byte_read + pool_acquire + pool_release + (file-refill) + per-FCB state). No sprint-change-proposal evaluation required per NFR-P4-5; Epic 19 still has headroom for Story 19.4's 0 B close-out.
...
- **v3 hardware UAT — PASS, 2026-05-22 15:21:52 (`~/Downloads/beastty-20260522-152152.bin`):** All three INCLUDEs on the +89 B v3 binary (26850 B) ran cleanly on real MicroBeast:
  - `INCLUDE P193INC1.FTH` → `---p193inc1-start---` / `42 ---p193inc1-end---` / clean ` ok`. **D1 v3 fix verified — no post-EOF garbage, no -13.**
  - `INCLUDE P193BNK2.FTH` → `---p193bnk2-start---` / `result=-1` / `---p193bnk2-end---` / clean ` ok`. **D2 fix verified — post-`0 BANK!` FIND-by-name on kernel words succeeded; bucket integrity invariant holds across `5 BANK! ... 0 BANK!` cycle.**
...
- **Binary delta = +22 B** (26761 → 26783) at v2 close; itemised in Dev Notes §"Pre-build byte itemisation" — D1 v2 flag cell (1 B) + check (6 B) + set (5 B) + reset (4 B) + D2 fix (6 B) = 22 B. Realised = itemised. Epic-19 cumulative = 198 B / 300 B (66%); ~102 B headroom remains for Story 19.4 close-out.
