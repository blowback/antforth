...
- **NFR-P4-3 (cross-bank dispatch overhead):** A cross-bank dispatch costs ≤ 400 Z80 T-states + the MMU port-write time, measured from the `RST $28` dispatch entry to the first instruction of the target body's code-field in the new bank. The ADR 19.5 per-opcode table (`docs/adr-19-5-cross-bank-dispatch.md:228`, ≈343 T dispatch-side for the option-C `RST $28` self-dispatch stub) is the authoritative measurement. _Re-baselined at the Epic 19.5 close-out (2026-06-08), per ADR 19.5 (accepted with this rider, Ant 2026-06-04, ADR `:3`). **SUPERSEDED original:** "A cross-bank call costs ≤ 60 Z80 T-states + the bank-switch time (NFR-P4-2), measured from call site's `JP <stub>` to first instruction of target body in the new bank." The original ≤60 T figure was an aspirational paper number — already unmet by the shipped Epic-18 `EXECUTE` path (≈420 T per-opcode) when it was codified — and is not achievable with a 2-cell frame + MMU lookup on the Z80 (ADR `:296`); option C at ≈343 T improves on shipped reality while exceeding the original number._
...
**Stories (shape CONFIRMED-AS-AMENDED by ADR 19.5, `docs/adr-19-5-cross-bank-dispatch.md`, Story 19.5.0 dev-pass 2026-06-04):**
- **Story 19.5.0 — ADR spike (zero binary delta). DONE.** Two decision records locked: **DR-1** — the "sentinel-trampoline layout-fragility" is actually **portal-window dictionary aliasing** (a colon body compiled at/above `$8000` vanishes mid-execution when the body maps a foreign bank over MMU slot 2; kernel-size shifts merely move the dictionary's `$8000` crossing point — knob-equivalence proven; H-A sentinel-collision / H-B emulator-defect / H-C trampoline-frame-bug all falsified; deterministic reproducer + dose-response matrix committed under `tests/`). **DR-2** — option **C** chosen on itemised evidence: self-dispatching 4-byte stub `[RST $28][bank][target]` + dispatch-site 2-cell frame + fixed-memory return thunk; ≈ −13 B net (retires EXIT_CODE sentinel discriminator, cross_bank_return trampoline, EXECUTE 3-way), 0 T per NEXT step; A re-itemised at ≈ +1990 B (320-site multiplier — the ~+250 B provisional was 8× under), B at ≈ −875 B but +28 T/step = +22.6% on the measured 124.1 T/iter loop anchor → both fail NFR-P4-1's no-measurable-regression clause. Root cause (b) subsumed by the thunk design (non-DOCOL targets return uniformly).
- **Story 19.5.1 — portal-aliasing guards + bank-N HERE COLD-init (H5) + reproducer regression slot** (was "trampoline stabilization" — DR-1 found no trampoline defect to fix). F1: `BANK!` window guard (~20 B) THROWs on a foreign-bank switch from window-resident code (converts the silent user-facing corruption class into a catchable error). F2: bank-table[1..28].here = `$8000` at COLD (~17 B; the H5 fix re-landed — unblocked now that the hang that forced its revert is explained). F3: Makefile regression slot for `tests/straddle_repro.fth.in` (PASS configuration passes AND HANG configuration still dies).
- **Story 19.5.2 — dispatch rework: self-dispatching RST stub + return thunk (option C).** Stub layout v2 (byte 0 `$EF`, byte 1 bank, bytes 2..3 target — size/capacity/NFR-P4-4 unchanged), `$0028` handler install at COLD, `xbank_thunk` + `xbank_restore`, EXECUTE fold-in (3-way retires), EXIT_CODE/`cross_bank_return` retirement, `BANK-OF` +1 offset, consumer/probe/doc migration per the ADR checklist; CATCH-cross-bank frame fix (+~20 B) designed here. Envelope −13..+25 B core + CATCH.
- **Story 19.5.3 — compiled-body verification.** (H5 moved to 19.5.1, its natural home.) Re-enable + verify 19.2 AC4/AC5 + 19.3 AC3/DOES> compiled-body probes (remove the `EXECUTE`-explicit reword + the manual `ALLOT`); banked NFR-P4-8 (CATCH-cross-bank) variant verifies 19.5.2's frame fix.
- **Story 19.5.4 — HW investigation + epic close-out.** ADR named assumptions: A1 straddle-reproducer PASS/HANG signature on real MicroBeast (DR-1 predicts identical death — the prior "HW passes" evidence was input-confounded); A2 `$0028` RST-vector claimability under MicroBeast CP/M; A3 Probe-19.2-F re-test under the C dispatch + F2 COLD-init (plausibly the same aliasing family) with the Probe-18.3-A2 fixed-memory-target bisect as first diagnostic. Full three-surface sweep; tag. NFR-P4-3 re-baseline (≤ 400 T + MMU port write, per ADR per-opcode tables) rides with ADR acceptance.
...
**Epic 19.5 summary:** ADR-first stabilization interlude. 5 stories (19.5.0 ADR spike [DONE] → 19.5.1 portal-aliasing guards + H5 → 19.5.2 RST-stub dispatch rework (option C) → 19.5.3 compiled-body verification → 19.5.4 HW + close-out); story shape finalized by ADR 19.5 (`docs/adr-19-5-cross-bank-dispatch.md`, 2026-06-04). Own binary envelope: **~+30..+100 B** (ADR-itemised: F1 ~20 + F2 ~17 + option-C net −13..+25 + CATCH ~20, ×1.25 discipline) — well under the pre-ADR ~+130–250 B provisional because option C's retirements (EXIT_CODE discriminator, cross_bank_return trampoline, EXECUTE 3-way) pay for most of its additions. Discharges all five Epic-19 architectural-debt anchors. Ships its own antforth 3.x point-release on close-out. Sprint-status row renames flagged by the ADR: `19-5-1-trampoline-stabilization-layout-fragility-fix` → portal-aliasing-guards shape, `19-5-3-compiled-body-verification-bank-n-here-cold-init-h5` → H5 reference moves to 19.5.1 (apply at 19.5.1 create-story time).
