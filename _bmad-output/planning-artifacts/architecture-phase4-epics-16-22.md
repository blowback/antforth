...
- (b) Command-line tail. ★ **CHOSEN** ★ — `antforth <portal-page> <bank-list>` (e.g. `antforth 24 35-3f`); defaults to `22 35-3F` (11 banks); probe-on-add via `+BANK`; banner shows the active-bank count.
...
> **Compiled-body dispatch behaviourally delivered — verified at the Epic 19.5 compiled-body verification pass (2026-06-07, status append):** the self-dispatching stub above is now proven to dispatch from a **compiled thread cell** (NEXT's `JP (HL)` into a banked word from another word's body), not only via explicit `EXECUTE`. FR-P4-15 intra-bank (probe-19.5.3-ac2), FR-P4-16 cross-bank-from-bank-0 north-star (probe-19.5.3-ac3), FR-P4-25 non-DOCOL DOVAR dispatch+return and intra-bank CREATE/DOES> (re-enabled probe-19.3-F/G), and the full banked NFR-P4-8 CATCH-cross-bank state-integrity variant (probe-19.5.3-ac6) all PASS under `iz-cpm-banking`. 0 B kernel delta (test+doc only). By-name source syntax for a bank-N callee from bank 0 still awaits Epic-20 bank-aware FIND (redesign §5.5); the probes inject the portable stub-xt via `[ COMPILE, ]`, exercising the dispatch path byte-identically.

> **Epic-19 architectural-debt anchors — DISCHARGED at the Epic 19.5 close-out (2026-06-08, antforth 3.0.4, status append):** Epic 19 shipped the verified bank-aware compiler *mechanism* around five cross-bank-dispatch debt anchors carried forward at the Epic-19 close-out. Epic 19.5 discharges all five (the six surface items below collapse to five root families — (c) and (d) are the single portal-window-aliasing / MMU-readback-float family per ADR 19.5 DR-1 + the DIV-1 divergence, not two independent defects):
> - **(a) DTC threading-through-stub-xt** (`NEXT`'s blind `JP (HL)` into a stub thread cell) → **discharged** by the option-C self-dispatching stub (byte 0 = `$EF`/`RST $28`, ADR 19.5 DR-2); behaviourally verified from a compiled thread cell at the Epic 19.5 compiled-body verification pass.
> - **(b) cross-bank trampoline assumes DOCOL/EXIT — non-DOCOL targets hang** (Probe-19.3-F) → **discharged** by the uniform `xbank_thunk`/`xbank_restore` return path (DOCOL and non-DOCOL alike); verified via re-enabled probe-19.3-F/G + the DOVAR dispatch+return probe.
> - **(c) sentinel-trampoline layout-fragility** → **reframed** by ADR 19.5 DR-1: the root cause was **portal-window dictionary aliasing**, NOT trampoline fragility (`feedback_iz_cpm_trampoline_fragility` SUPERSEDED). The `BANK!` window guard (`-273`) **CONTAINS** the user-facing class; **abolition** (per-wordlist bank field / bank-aware FIND) is **Epic 20** (redesign §5.5). Status: **contained, not abolished; owning fix Epic 20.**
> - **(d) intra-bank-EXECUTE-into-slot-2 HW gap** (Probe-19.2-F) → **discharged**: confirmed on silicon (intra-bank EXECUTE into slot 2 → result; the old 19.2-F hang shape is dead) once the DIV-1 MMU-port-readback float was fixed — it was the same readback-float family, not a distinct defect.
> - **(e) CATCH-around-cross-bank reboot** → **discharged** by the CR-F1 caught-`THROW` MMU + current-bank + dictionary-triple restore; verified by the full banked NFR-P4-8 CATCH-cross-bank variant.
> - **(f) bank-N HERE COLD-init (H5)** → **discharged** by the F2 bank-table[1..28].here = `$8000` COLD-init (re-landed at the Epic 19.5 portal-aliasing-guards story). [This is the sixth surface anchor from the Epic-19 carry-forward list; the epic summary counts (c)+(d) as the single layout/HW-aliasing family, so six surface items → five root families.]
>
> Residual fences (NOT bugs to beat — Epic 20 owns the abolition): cross-bank pointer reads (FR-P4-26), cross-bank-from-bank-N bodies, and cross-bank DOES> bodies remain `BANK!`-guarded documented hazards (redesign §5.4 / §5.5). Bank-N words are not yet `FIND`-able by name from another bank (the DR-1 residual) — contained by the guard, owning fix Epic 20.
...
**Architectural impact:** Story 18.1 (`bank-table[]` allocator) inherits the 4-byte allocation unit; Story 18.3 (descriptor-stub allocator + `EXECUTE` chokepoint) inherits the v1 byte-0-bank / byte-1-JP layout for its dispatch decoder (**superseded by Layout v2 above (Epic 19.5 dispatch-rework story): byte 0 = `$EF`/`RST $28`, byte 1 = signed `target_bank`**). NFR-P4-4 (per-stub ≤5 B) holds with margin. `BANK-OF` (FR-P4-5) is implemented as a one-byte read at xt+1 (byte 1 = signed `target_bank` under Layout v2 — `src/banking.asm:994`; +1 B vs v1's xt+0, the "essentially free" property survives; cited in Important Decisions :155).
...
- **(i) no args** → apply defaults `22 35-3F` (0x22 portal + 11 banks at 0x35..0x3F per redesign §6). No warning; the absence of args is a normal usage pattern (the user wants the default config). Boot continues with the default bank set.
...
#### F5 — Five architecture-stage open questions captured as `TODO(P4-arch)` markers
...
- All 5 remaining architecture-stage open questions from redesign §9 closed by Story 16.4 (2026-05-14): §9.1 → PD-P4-15 (CODE-words fixed-memory-only); §9.3 → PD-P4-14 (CL parser warn-and-continue); §9.4 → PD-P4-13 (`+BANK` past cap raises `ABORT" cap?"`); §9.5 → PD-P4-11 (4-byte stub); §9.6 → PD-P4-12 (R-stack documented-gotcha). §9.2 emulator vendor pick CLOSED by Story 16.3 (2026-05-13). §9.7 flat-build CLOSED 2026-05-10 as non-MVP. See Findings F5 closure paragraph for the close-out summary.
